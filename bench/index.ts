...
async function main(): Promise<void> {
  ...
  out.push('path is **~5× faster than a full Spineless rebuild** — it');
  out.push('skips the O(tree) `init()` layout compute. `buildAppendFragment`');
  out.push('now emits just the appended subtree (O(subtree)) against the');
  out.push('runtime grammar as a boundary — no whole-tree rebuild. The');
  out.push('remaining gap to the imperative `calculateLayout()` is');
  out.push('`buildRemoveFragment`, which still rebuilds the grammar');
  out.push('O(tree) to diff out the removed fields; the matching');
  out.push('O(subtree) removal is the next optimisation.');
  out.push('');
